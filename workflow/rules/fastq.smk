rule get_fastq_pe:
    output:
        temp("results/{ref_name}/data/fastq/{sample}/{run}_1.fastq.gz"),
        temp("results/{ref_name}/data/fastq/{sample}/{run}_2.fastq.gz")
    params:
        outdir = os.path.join(workflow.default_remote_prefix, "results/{ref_name}/data/fastq/{sample}/")
    conda:
        "../envs/fastq2bam.yml"
    threads:

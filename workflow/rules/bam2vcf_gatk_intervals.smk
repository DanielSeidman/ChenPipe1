...
rule bam2gvcf:
    ...
    resources:
        ...
        reduced = lambda wildcards, attempt: attempt * ((mem_mb) - 3000)  # this is the maximum amount given to java
    log:
...
rule concat_gvcfs:
    ...
    resources:
        mem_mb = lambda wildcards, attempt: attempt * resources['gatherVcfs']['threads'] * 2000,   # this is the overall memory requested
        tmpdir = get_big_temp
    conda:
        "../envs/bcftools.yml"
    shell:
...
rule gvcf2DB:
    ...
    resources:
        mem_mb = lambda wildcards, attempt: attempt * resources['gvcf2DB']['threads'] * 2000,   # this is the overall memory requested
        reduced = lambda wildcards, attempt: int(attempt * resources['gvcf2DB']['threads'] * 2000 * 0.80) # this is the maximum amount given to java
    log:
...
rule DB2vcf:
    ...
    resources:
        mem_mb = lambda wildcards, attempt: attempt * resources['DB2vcf']['threads'] * 2000,   # this is the overall memory requested
        reduced = lambda wildcards, attempt: attempt * ((resources['DB2vcf']['threads'] * 2000) - 3000),  # this is the maximum amount given to java
        tmpdir = get_big_temp
    log:
...
rule filterVcfs:
    ...
    conda:
        "../envs/bam2vcf.yml"
    resources:
        mem_mb = lambda wildcards, attempt: attempt * resources['filterVcfs']['threads'] * 2000   # this is the overall memory requested
    log:
